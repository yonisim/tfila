...
function is_slihot_days(date){
    return is_between_dates(date, "2025-08-26T00:00", "2025-09-30T23:00");
}
...
function is_rosh_hashana_eve(date){
    return is_between_dates(date, "2025-09-22T00:01", "2025-09-23T02:00");
}

function is_rosh_hashana(date){
    return is_between_dates(date, "2025-09-23T02:01", "2025-09-24T02:00");
}

function is_rosh_hashana_b(date){
    return is_between_dates(date, "2025-09-24T02:01", "2025-09-24T19:00");
}

function is_gedalia(date){
    return is_between_dates(date, "2025-09-25T02:00", "2025-09-25T23:00");
}
...
function is_kipur_eve(date){
    return is_between_dates(date, "2025-10-01T02:00", "2025-10-01T22:00");
}

function is_kipur(date){
    return is_between_dates(date, "2025-10-01T18:00", "2025-10-02T19:00");
}
...
function is_shabat_zachor(date){
    return is_between_dates(date, "2026-02-27T16:00", "2026-02-28T17:00");
}
...
function get_specific_single_page(current_date){
    ...
    if(is_pesach_first_chag(current_date_obj)){
        ...
        if(is_in_weekdays(current_date_obj, [5])){
...
function get_date_plus_days(current_date, add_days){
    var result = new Date(current_date);
    result.setDate(current_date.getDate() + add_days);
    return result;
}

function get_date_plus_minutes(current_date, add_minutes){
    var result = new Date(current_date);
...
function normalize_prayer_slot_for_display(value){
    if (value == null || value === ''){
        return '';
    }
    if (Array.isArray(value)){
        return value.filter(function (x){ return x != null && x !== ''; }).join(' / ');
    }
    return String(value);
}

/** One row from mincha_maariv payload (canonical keys mincha / maariv). */
function normalize_mincha_maariv_entry(entry){
    if (!entry){
        return { mincha: '', maariv: '' };
    }
    var maarivRaw = get_single_prayer_times_from_date_obj(entry, 'maariv');
    var minchaRaw = get_single_prayer_times_from_date_obj(entry, 'mincha');
    return {
        mincha: normalize_prayer_slot_for_display(minchaRaw),
        maariv: normalize_prayer_slot_for_display(maarivRaw),
    };
}
...
function get_next_week_mincha_maariv_for_footer(current_date){
    var anchorSundays = [
        ...
        get_week_start_date(current_date),
        get_date_plus_days(get_next_week_start_date(current_date), -7),
        get_date_plus_days(get_next_week_start_date(current_date), 7),
    ];
    var j;
    var bestIncomplete = null;
    for (j = 0; j < anchorSundays.length; j++){
        var key = get_date_from_Date(anchorSundays[j]);
        var entry = week_times[key];
        var row = normalize_mincha_maariv_entry(entry);
        if (row.mincha){
            return row;
        }
        if (!bestIncomplete && (row.mincha || row.maariv)){
            bestIncomplete = row;
        }
    }
    var start = get_next_week_start_date(current_date);
    var i;
    for (i = 0; i < 7; i++){
        var d = get_date_plus_days(start, i);
        entry = week_times[get_date_from_Date(d)];
        if (!entry){
            continue;
        }
        row = normalize_mincha_maariv_entry(entry);
        if (row.mincha){
            return row;
        }
        if (!bestIncomplete && (row.mincha || row.maariv)){
    ...
    return normalize_mincha_maariv_entry(get_week_times(current_date));
}
...
function show_slichot(date){
    if(is_between_dates(date, '2025-09-14T10:00', '2025-10-01T18:00')){
...
async function show_footer_custom_message_if_needed(current_date, into_elem_id, caller_slee_seconds=60){
    ...
    if(is_between_dates(current_date, '2025-12-30T05:00', '2025-12-30T17:20')){
        messages.push('צום עשרה בטבת | מנחה: 16:15 | ערבית וצאת הצום: 17:05');
        show_footer = true;
    }
    ...
    if(is_rosh_chodesh(current_date)){
        messages.push('ראש חודש   |   יעלה ויבוא');
        show_footer = true
    }
...
/* Default time column: equal-width slots across the strip (desktop), still wraps on tight widths. */
...
/** One time + caption column; use wrapperClass for optional hidden-element / toggle classes (e.g. slichot, arvit-8). */
function tz_tfilot_grouped_time_column_html(options){
    var wrapper = options.wrapperClass || TZ_TF_COL_DEFAULT;
    var timePart = options.timeHtml != null ? options.timeHtml : tz_tfilot_time_span_html(options.timeText, options.timeId);
    var capPart =
        options.captionHtml != null
            ? options.captionHtml
...
/** Horizontal strip of columns (time slots). */
function tz_tfilot_grouped_time_strip_html(columnsHtml){
    return '<div class="' + TZ_TF_STRIP + '">' + columnsHtml + '</div>';
}

/** Same as tz_tfilot_grouped_time_strip_html but centers contents and doesn't expand to full width. */
function tz_tfilot_grouped_time_strip_center_html(columnsHtml){
...
/** Empty container for runtime mincha-gedola rows (13:30 logic). */
function tz_tfilot_mincha_dynamic_prepend_container_html(){
    return (
        '<div id="mincha-dynamic-prepend" class="flex min-w-0 flex-wrap items-end gap-x-4 gap-y-2 sm:gap-x-6"></div>'
    );
}

function create_tfilot_mincha_dynamic_row_html(time, label){
...
function get_tfilot_shacharit_grouped_card_inner_html(current_date){
    var slots = [
    ...
    if (is_between_dates(current_date, '2025-09-14T10:00', '2025-10-01T18:00')){
        slots.push(
            tz_tfilot_grouped_time_column_html({
                wrapperClass:
                    'hidden-element slichot flex min-w-0 flex-1 basis-0 flex-col items-center gap-0.5',
                timeText: '06:30',
                timeId: 'slichot',
                captionText: 'סליחות (משוער)',
                captionMaxClass: TZ_TF_CAP_MD,
            })
        );
    }
    slots.push(
        ...
        tz_tfilot_grouped_time_column_html({
            wrapperClass:
                'hidden-element friday-shacharit flex min-w-0 flex-1 basis-0 flex-col items-center gap-0.5',
            timeText: '08:30',
            timeId: 'shacharit_main',
            captionText: 'שחרית ג',
...
function get_tfilot_mincha_grouped_card_inner_html(){
    var slots = [
        ...
        tz_tfilot_grouped_time_column_html({
            wrapperClass:
                'hidden-element mincha-gedola flex min-w-0 flex-1 basis-0 flex-col items-center gap-0.5',
            timeText: '13:15',
...
function get_tfilot_shabat_mincha_grouped_card_inner_html(){
    var slots = [
        tz_tfilot_grouped_time_column_html({
            timeText: '',
            timeId: 'mincha-shabat-a',
            captionText: 'מנחה א',
            captionMaxClass: TZ_TF_CAP_TIGHT,
        }),
        tz_tfilot_grouped_time_column_html({
            timeText: '',
            timeId: 'mincha-shabat-b',
            captionText: 'מנחה ב',
            captionMaxClass: TZ_TF_CAP_TIGHT,
        }),
        tz_tfilot_grouped_time_column_html({
            timeText: '',
            timeId: 'mincha-shabat-c',
            captionText: 'מנחה קטנה',
            captionMaxClass: TZ_TF_CAP_TIGHT,
        }),
    ];
...
/** Same rule as set_arvit_times: 20:00 column only when first ערבית is not after 19:41. */
function tfilot_show_arvit_20_column(current_date, arvit_time){
    ...
    return !is_after_time(arvit_date, '19:41');
}

function get_tfilot_arvit_grouped_card_inner_html(current_date, arvit_time){
    var show_arvit_20 = tfilot_show_arvit_20_column(current_date, arvit_time);
    var slots = [
        tz_tfilot_grouped_time_column_html({
            timeText: '',
            timeId: 'arvit-regulr-days',
            captionText: 'ערבית א',
        }),
    ];
    if (show_arvit_20){
...
function fill_tfilot_prayer_times_grouped_cards(current_date, arvit_time){
    set_element_html('tfilot-prayer-card-shacharit', get_tfilot_shacharit_grouped_card_inner_html(current_date));
    set_element_html('tfilot-prayer-card-mincha', get_tfilot_mincha_grouped_card_inner_html());
    set_element_html('tfilot-prayer-card-arvit', get_tfilot_arvit_grouped_card_inner_html(current_date, arvit_time));
}

/** Friday single-page: helper for an equal-width horizontal cards row. */
function tz_equal_width_cards_row_html(cardIds){
    var cards = [];
    for (var i = 0; i < cardIds.length; i++){
        cards.push(
            '<div class="tz-glass-card flex min-w-0 flex-1 basis-0 flex-col gap-3 rounded-xl border-r-8 border-primary p-5 shadow-glass" id="' +
                cardIds[i] +
                '"></div>'
        );
    }
...
/* Friday: captions inside horizontal cards should stay on one line when possible. */
...
/** Under מנחה: תהלים, פרקי אבות, ערבית, ערבית ב — same glass cards as ציר מעל. Class names preserve shabat_chazon_adaptions. */
function get_shabat_afternoon_horizontal_cards_html(){
    var col =
...
function get_friday_shacharit_card_inner_html(current_date){
    var inner = '';
    if (is_10_tevet_friday(current_date)){
        inner +=
            '<div class="mb-3 border-b border-primary/30 pb-3">' +
            tz_tfilot_grouped_time_strip_html(
                tz_tfilot_grouped_time_column_html({
                    timeText: '05:06',
                    captionText: 'כניסת הצום',
                    captionMaxClass: TZ_TF_CAP_MD,
                })
            ) +
            '</div>';
...
function fill_friday_prayer_grouped_cards(current_date){
    /* Optional: if we ever want to render the “erev shabbat” row dynamically, keep this helper around:
       tz_equal_width_cards_row_html(['friday-prayer-card-mincha-gedola','friday-prayer-card-hadlakat','friday-prayer-card-mincha-kabalat']) */
    set_element_html('friday-prayer-card-shacharit', get_friday_shacharit_card_inner_html(current_date));
    set_element_html('friday-prayer-card-mincha-gedola', get_friday_mincha_gedola_card_inner_html());
...
/** מנין פלג — same typography as prayer cards (no legacy grid). */
function get_friday_plag_minyan_card_inner_html(){
    return (
        ...
        tz_tfilot_grouped_time_strip_center_html(
            tz_tfilot_grouped_time_column_html({
                timeText: '',
                timeId: 'kabalat-shabat-early-mincha',
                captionText: 'מנחה וקבלת שבת מוקדמת',
                captionMaxClass: TZ_TF_CAP_NOWRAP,
            })
        ) +
        '</div>' +
        ...
        tz_tfilot_grouped_time_strip_center_html(
            tz_tfilot_grouped_time_column_html({
                timeText: '',
                timeId: 'kabalat-shabat-early',
                captionText: 'פלג המנחה',
                captionMaxClass: TZ_TF_CAP_NOWRAP,
            })
        ) +
        '</div>' +
...
function apply_friday_shabat_eve_card_times(current_date, shabat_in){
    set_element_html('hadlakat-nerot', shabat_in);
    if (is_purim(current_date)){
        hide_element('mincha_shabat_eve');
        set_element_html('mincha_shabat_eve', add_minutes_to_time(shabat_in, 20));
    }
    if (!is_10_tevet_friday(current_date)){
...
async function present_prayer_times_single_page(current_date){
    ...
    fill_tfilot_prayer_times_grouped_cards(current_date, arvit_time);
...
function set_arvit_times(current_date, arvit_time){
    ...
    if(is_after_time(arvit_date, "19:41")){
...
async function present_rosh_hashana_b_prayer_times(current_date){
    ...
    var gedalia_date = get_date_plus_days(current_date, 1);
...
function shabat_zachor_adaptions(){
    var zachor_element = document.getElementById('shabat-zachor');
    toggle_element_show(zachor_element, false);
}
...
async function embed_next_week_prayer_times(current_date, parent_elem_id, plus_days=0) {
    load_html_into_page_elem_end('next_week_prayer_times_embedded_with_title.html', parent_elem_id, () => {
        var t = get_next_week_mincha_maariv_for_footer(current_date);
        set_element_html('mincha-regulr-days', t.mincha);
        set_element_html('arvit-regulr-days', t.maariv);
    });
}
...
function populate_friday_prayer_times(current_date){
    var this_week_times = get_week_times(current_date);
    var this_shabat_times = get_shabat_times(current_date);
    var shabat_in = this_shabat_times["in"];
    document.getElementById("prayer-times-title-parasha").innerText = this_shabat_times['parasha'];
    ...
    return fetch('./html/prayer_times_friday_single_page.html')
        .then(function (res){
            return res.text();
        })
        .then(function (html){
            set_element_html('friday_prayers', html);
            fill_friday_prayer_grouped_cards(current_date);
            show_slichot(current_date);
            var elements = document.getElementsByClassName('friday-shacharit');
            for (var i = 0; i < elements.length; i++){
                elements[i].classList.add('show-element');
            }
            return show_shabat_eve_times(current_date, shabat_in, 'friday_prayers');
        });
}
...
function show_minyan_plag(current_date) {
    ...
    var plagRoot = document.getElementById('plag');
    if (!plagRoot){
        return;
    }
    plagRoot.innerHTML = get_friday_plag_minyan_card_inner_html();
    ...
    if(!kabalat_shabat_early_mincha){
        ...
        kabalat_shabat_early_mincha = add_minutes_to_time(rounded, -20);
    }
