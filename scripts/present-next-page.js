...
import {
    ...
    is_present_pesach_eve, is_pesach_eve, is_pesach_first_chag, is_pesach_7_eve, is_pesach_7,
    is_shabat_hagadol_tashpa,
    is_present_memorial_day, is_present_atzmaut,
...
import {
    TZ_TF_CAP_NOWRAP,
    SHABAT_DAY_CARD_SHELL, SHAVUOT_KABALAT_YELADIM_CARD_SHELL, SHAVUOT_COMPACT_TF_CARD_SHELL,
    tz_icon_sun_svg, tz_icon_moon_svg, tz_icon_shabbat_candles_svg, tz_icon_kiddush_cup_svg,
    tz_icon_child_svg, tz_icon_book_open_svg, tz_icon_scroll_svg, tz_icon_people_svg,
    tz_tfilot_grouped_time_column_html, tz_tfilot_grouped_time_strip_html,
    tz_tfilot_grouped_time_strip_center_html, tz_tfilot_grouped_time_strip_center_html_beside_icon,
    tz_tfilot_grouped_prayer_row_html,
    ...
    get_rosh_hashana_eve_page_grid_html,
...
// ─── Runtime-dependent predicates (keep here — need day_times / shabat_times) ──
...
function apply_friday_shabat_eve_card_times(current_date, shabat_in){
    ...
    if (is_purim(current_date)){
        ...
        set_element_html('kabalat_shabat', add_minutes_to_time(shabat_in, 20));
    }
...
async function present_rosh_hashana_a_prayer_times(current_date){
    set_element_html('rosh_hashana_a_grid', get_rosh_hashana_a_page_grid_html());
    present_day_times(current_date);
    return sleep_seconds(10*60);
}
...
async function present_rosh_hashana_b_shabat_shuva_eve_prayer_times(current_date){
    set_element_html('rosh_hashana_b_shabat_shuva_grid', get_rosh_hashana_b_shabat_shuva_eve_page_grid_html());
    present_day_times(current_date);
    return sleep_seconds(wait_seconds*10);
}
...
async function populate_friday_prayer_times(current_date){
    var this_shabat_times = get_shabat_times(current_date);
    var shabat_in = this_shabat_times['in'];
    var parasha = this_shabat_times['parasha'];
    if (is_10_tevet_friday(current_date)) {
        parasha += ' (עשרה בטבת)';
    }
...
function show_minyan_plag(current_date) {
    if (!document.getElementById('plag')) { return; }
    set_element_html('plag', get_friday_plag_minyan_card_inner_html());
    apply_plag_minyan_card_times(current_date);
}

function apply_plag_minyan_card_times(current_date) {
    var day = is_in_weekdays(current_date, [4]) ? addDays(current_date, 1) : current_date;
    ...
    if (!kabalat_shabat_early_mincha) {
        kabalat_shabat_early_mincha = add_minutes_to_time(round_to_five(plag, true), -20);
    }
